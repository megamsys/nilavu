...
module Tilt
  ...
  class ES6ModuleTranspilerTemplate < Tilt::Template
    ...
    def whitelisted?(path)
      ...
      @@whitelisted ||= Set.new(
        ["nilavu/models/nav-item",
          "nilavu/models/user-action",
          "nilavu/routes/nilavu",
          "nilavu/models/category",
          "nilavu/models/trust-level",
          "nilavu/models/site",
          "nilavu/models/user",
      ...
      @@whitelisted.include?(path) || path =~ /nilavu\/mixins/
    end
    ...
    def evaluate(scope, locals, &block)
      ...
      if whitelisted?(scope.logical_path) &&
        scope.logical_path =~ /(nilavu|admin)\/(controllers|components|views|routes|mixins|models)\/(.*)/
        ...
        if require_name !~ /\-test$/ && require_name !~ /^nilavu\/plugins\//
          ...
          if result != "PostMenuView"
            @output << "\n\nNilavu.#{result} = require('#{require_name}').default;\n"
          end
        end
      end
    ...
    def module_name(root_path, logical_path)
      ...
      if root_path =~ /(.*\/#{root_base}\/plugins\/[^\/]+)\//
        ...
        plugin = Nilavu.plugins.find {|p| p.path == plugin_path }
        path = "nilavu/plugins/#{plugin.name}/#{logical_path.sub(/javascripts\//, '')}" if plugin
      end
