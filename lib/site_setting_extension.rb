...
module SiteSettingExtension
  ...
  def provider
    @provider ||= SiteSettings::LocalProcessProvider.new
  end
  ...
  # just like a setting, except that it is available in javascript via NilavuSession
  ...
  def add_override!(name, val)
    ...
    if type == types[:enum]
      ...
      if enum_class(name)
        raise Nilavu::InvalidParameters.new(:value) unless enum_class(name).valid_value?(val)
      else
        raise Nilavu::InvalidParameters.new(:value) unless choices[name].include?(val)
      end
    end
    ...
    if v = validators[name]
      ...
      unless validator.valid_value?(val)
        raise Nilavu::InvalidParameters.new(validator.error_message)
      end
    end
  ...
  def set(name, value)
    if has_setting?(name) && is_valid_data?(name, value)
      ...
      Nilavu.request_refresh! if requires_refresh?(name)
    else
