...
class UserDestroyer
  ...
  def initialize(actor)
    @actor = actor
    raise Nilavu::InvalidParameters.new('acting user is nil') unless @actor and @actor.is_a?(User)
  end

  # Returns false if the user failed to be deleted.
  # Returns a frozen instance of the User if the delete succeeded.
  def destroy(user, opts={})
    raise Nilavu::InvalidParameters.new('user is nil') unless user and user.is_a?(User)
    ...
    User.transaction do
      ...
      if opts[:delete_posts]
        user.posts.each do |post|
          ...
          PostDestroyer.new(@actor.staff? ? @actor : Nilavu.system_user, post).destroy
      ...
      user.post_actions.each do |post_action|
        post_action.remove_act!(Nilavu.system_user)
      end
      ...
      user.destroy.tap do |u|
        if u
          ...
          categories.each do |c|
            c.user_id = Nilavu.system_user.id
            c.save!
            if topic = Topic.with_deleted.find_by(id: c.topic_id)
              topic.try(:recover!)
              topic.user_id = Nilavu.system_user.id
              topic.save!
            end
          end

          StaffActionLogger.new(@actor == user ? Nilavu.system_user : @actor).log_user_deletion(user, opts.slice(:context))
