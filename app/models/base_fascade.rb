class BaseFascade
  ...
  def api_request(jparams, jlaz, jmethod, passthru = false)
    ...
    unless passthru
      fail MissingAPIArgsError, ':email and :api_key required.' unless jparams.key?(:email && :api_key)
    end
  ...
  def run_now(_swallow_exception = false, jlaz, jmethod, jparams)
    ...
    unless api_jlaz.respond_to?(jmethod)
      ...
      fail UnsupportedAPI, "#{jlaz}.#{jmethod}, try adding before you can use it."
    end
