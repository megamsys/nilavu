  ...
  actions: {
    lookup: function () {
      ...
      if (!this.get("location")) {
        Nilavu.ajax("/admin/users/ip-info", {
          data: { ip: this.get("ip") }
      ...
      if (!this.get("other_accounts")) {
        ...
        Nilavu.ajax("/admin/users/total-others-with-same-ip", { data }).then(function (result) {
    ...
    deleteOtherAccounts: function() {
      ...
      bootbox.confirm(I18n.t("ip_lookup.confirm_delete_other_accounts"), I18n.t("no_value"), I18n.t("yes_value"), function (confirmed) {
        if (confirmed) {
          ...
          Nilavu.ajax("/admin/users/delete-others-with-same-ip.json", {
            type: "DELETE",
            data: {
              "ip": self.get("ip"),
