const SiteSetting = Nilavu.Model.extend({
  overridden: function() {
...
SiteSetting.reopenClass({
  findAll() {
    return Nilavu.ajax("/admin/site_settings").then(function (settings) {
  ...
  update(key, value) {
    const data = {};
    data[key] = value;
    return Nilavu.ajax("/admin/site_settings/" + key, { type: 'PUT', data });
  }
});
