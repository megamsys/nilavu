...
const ColorScheme = Nilavu.Model.extend(Ember.Copyable, {
  ...
  save: function(opts) {
    ...
    return Nilavu.ajax("/admin/color_schemes" + (this.id ? '/' + this.id : '') + '.json', {
  ...
  destroy: function() {
    if (this.id) {
      return Nilavu.ajax("/admin/color_schemes/" + this.id, { type: 'DELETE' });
    }
  }
...
ColorScheme.reopenClass({
  findAll: function() {
    ...
    Nilavu.ajax('/admin/color_schemes').then(function(all) {
