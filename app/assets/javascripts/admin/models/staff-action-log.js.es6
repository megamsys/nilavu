...
const StaffActionLog = Nilavu.Model.extend({
  ...
  formattedDetails: function() {
    ...
    if (!this.get('useModalForDetails')) {
      if (this.get('details')) formatted += Nilavu.Utilities.escapeExpression(this.get('details')) + '<br/>';
    }
    return formatted;
  ...
  format: function(label, propertyName) {
    if (this.get(propertyName)) {
      return ('<b>' + I18n.t(label) + ':</b> ' + Nilavu.Utilities.escapeExpression(this.get(propertyName)) + '<br/>');
    } else {
      return '';
    }
  },
...
StaffActionLog.reopenClass({
  ...
  findAll: function(filters) {
    return Nilavu.ajax("/admin/logs/staff_action_logs.json", { data: filters }).then(function(staff_actions) {
