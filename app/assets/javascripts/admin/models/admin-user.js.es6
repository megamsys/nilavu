...
const AdminUser = Nilavu.User.extend({
  ...
  generateApiKey() {
    const self = this;
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/generate_api_key", {
      type: 'POST'
  ...
  groupRemoved(groupId) {
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/groups/" + groupId, {
      type: 'DELETE'
    }).then(() => this.set('groups.[]', this.get('groups').rejectBy("id", groupId)));
  },

  revokeApiKey() {
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/revoke_api_key", {
      type: 'DELETE'
  ...
  deleteAllPosts() {
    const user = this,
          ...
          buttons = [{
            ...
            "callback": function() {
              Nilavu.ajax("/admin/users/" + user.get('id') + "/delete_all_posts", {
                type: 'PUT'
  ...
  revokeAdmin() {
    const self = this;
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/revoke_admin", {
      type: 'PUT'
    }).then(function() {
      self.setProperties({
        admin: false,
  ...
  grantAdmin() {
    const self = this;
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/grant_admin", {
      type: 'PUT'
    }).then(function() {
      self.setProperties({
        admin: true,
  ...
  revokeModeration() {
    const self = this;
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/revoke_moderation", {
      type: 'PUT'
    }).then(function() {
      self.setProperties({
        moderator: false,
  ...
  grantModeration() {
    const self = this;
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/grant_moderation", {
      type: 'PUT'
    }).then(function() {
      self.setProperties({
        moderator: true,
  ...
  refreshBrowsers() {
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/refresh_browsers", {
      type: 'POST'
  ...
  approve() {
    const self = this;
    return Nilavu.ajax("/admin/users/" + this.get('id') + "/approve", {
      type: 'PUT'
    }).then(function() {
      self.setProperties({
        can_approve: false,
        approved: true,
        approved_by: Nilavu.User.current()
      });
    });
  },
  ...
  suspend(duration, reason) {
    return Nilavu.ajax("/admin/users/" + this.id + "/suspend", {
      type: 'PUT',
  ...
  activate() {
    return Nilavu.ajax('/admin/users/' + this.id + '/activate', {
      type: 'PUT'
    }).then(function() {
      window.location.reload();
    }).catch(function(e) {
  ...
  deactivate() {
    return Nilavu.ajax('/admin/users/' + this.id + '/deactivate', {
      type: 'PUT'
    }).then(function() {
      window.location.reload();
    }).catch(function(e) {
  ...
  block() {
    ...
    const performBlock = function() {
      ...
      return Nilavu.ajax('/admin/users/' + user.id + '/block', {
        type: 'PUT'
      }).then(function() {
        window.location.reload();
      }).catch(function(e) {
  ...
  anonymize() {
    ...
    const performAnonymize = function() {
      return Nilavu.ajax("/admin/users/" + user.get('id') + '/anonymize.json', {
        type: 'PUT'
      }).then(function(data) {
        if (data.success) {
          if (data.username) {
...
AdminUser.reopenClass({
  ...
  find(user_id) {
    return Nilavu.ajax("/admin/users/" + user_id + ".json").then(result => {
  ...
  findAll(query, filter) {
    return Nilavu.ajax("/admin/users/list/" + query + ".json", {
      data: filter
    }).then(function(users) {
