...
const FlaggedPost = Post.extend({
  ...
  canDeleteAsSpammer: function() {
    return Nilavu.User.currentProp('staff') && this.get('flaggedForSpam') && this.get('user.can_delete_all_posts') && this.get('user.can_be_deleted');
  }.property('flaggedForSpam'),

  deletePost: function() {
    if (this.get('post_number') === 1) {
      return Nilavu.ajax('/t/' + this.topic_id, { type: 'DELETE', cache: false });
    } else {
      return Nilavu.ajax('/posts/' + this.id, { type: 'DELETE', cache: false });
    }
  },

  disagreeFlags: function () {
    return Nilavu.ajax('/admin/flags/disagree/' + this.id, { type: 'POST', cache: false });
  },

  deferFlags: function (deletePost) {
    return Nilavu.ajax('/admin/flags/defer/' + this.id, { type: 'POST', cache: false, data: { delete_post: deletePost } });
  },

  agreeFlags: function (actionOnPost) {
    return Nilavu.ajax('/admin/flags/agree/' + this.id, { type: 'POST', cache: false, data: { action_on_post: actionOnPost } });
  },
...
FlaggedPost.reopenClass({
  findAll: function (filter, offset) {
    ...
    return Nilavu.ajax('/admin/flags/' + filter + '.json?offset=' + offset).then(function (data) {
      // users
      var userLookup = {};
