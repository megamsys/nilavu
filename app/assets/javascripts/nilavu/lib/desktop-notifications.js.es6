...
import { onPageChange } from 'nilavu/lib/page-tracker';
...
function init(messageBus) {
  ...
  try {
    ...
    Em.Logger.info('Nilavu desktop notifications are disabled - localStorage denied.');
    return;
  }

  if (!("Notification" in window)) {
    Em.Logger.info('Nilavu desktop notifications are disabled - not supported by browser');
    return;
  }
...
function onNotification(data) {
  ...
  const notificationTitle = I18n.t(i18nKey(data.notification_type), {
     site_title: Nilavu.SiteSettings.title,
     topic: data.topic_title,
     username: data.username
  });

  const notificationBody = data.excerpt;
  const notificationIcon = Nilavu.SiteSettings.logo_small_url || Nilavu.SiteSettings.logo_url;
  const notificationTag = "discourse-notification-" + Nilavu.SiteSettings.title + "-" + data.topic_id;
  ...
  requestPermission().then(function() {
    ...
    function clickEventHandler() {
      NilavuURL.routeTo(data.post_url);
...
function i18nKey(notification_type) {
  return "notifications.popup." + Nilavu.Site.current().get("notificationLookup")[notification_type];
}
