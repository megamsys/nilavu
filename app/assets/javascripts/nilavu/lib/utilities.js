...
Nilavu.Utilities = {
  ...
  **/
  ...
  // unsuitable for many of Nilavu's uses. Use `Handlebars.Utils.escapeExpression`
  ...
  avatarUrl: function(template, size) {
    if (!template) { return ""; }
    var rawSize = Nilavu.Utilities.getRawSize(Nilavu.Utilities.translateSize(size));
  ...
  avatarImg: function(options) {
    var size = Nilavu.Utilities.translateSize(options.size);
    var url = Nilavu.Utilities.avatarUrl(options.avatarTemplate, size);
    ...
    return "<img alt='' width='" + size + "' height='" + size + "' src='" + Nilavu.getURLWithCDN(url) + "' class='" + classes + "'" + title + ">";
  },

  tinyAvatar: function(avatarTemplate, options) {
    return Nilavu.Utilities.avatarImg(_.merge({avatarTemplate: avatarTemplate, size: 'tiny' }, options));
  },

  postUrl: function(slug, topicId, postNumber) {
    var url = Nilavu.getURL("/t/");
    if (slug) {
      url += slug + "/";
    } else {
      url += 'topic/';
  ...
  userUrl: function(username) {
    return Nilavu.getURL("/users/" + username.toLowerCase());
  },
  ...
  caretRowCol: function(el) {
    var caretPosition = Nilavu.Utilities.caretPosition(el);
  ...
  validateUploadedFiles: function(files, bypassNewUserRestriction) {
    ...
    var type = Nilavu.Utilities.isAnImage(upload.name) ? 'image' : 'attachment';

    return Nilavu.Utilities.validateUploadedFile(upload, type, bypassNewUserRestriction);
  },

  validateUploadedFile: function(file, type, bypassNewUserRestriction) {
    // check that the uploaded file is authorized
    if (!Nilavu.Utilities.authorizesAllExtensions() &&
        !Nilavu.Utilities.isAuthorizedUpload(file)) {
      var extensions = Nilavu.Utilities.authorizedExtensions();
    ...
    if (!bypassNewUserRestriction) {
      ...
      if (!Nilavu.User.current().isAllowedToUploadAFile(type)) {
  ...
  authorizesAllExtensions: function() {
    return Nilavu.SiteSettings.authorized_extensions.indexOf("*") >= 0;
  },

  isAuthorizedUpload: function(file) {
    if (file && file.name) {
      var extensions = _.chain(Nilavu.SiteSettings.authorized_extensions.split("|"))
  ...
  authorizedExtensions: function() {
    return _.chain(Nilavu.SiteSettings.authorized_extensions.split("|"))
            .reject(function(extension) { return extension.indexOf("*") >= 0; })
  ...
  getUploadMarkdown: function(upload) {
    if (Nilavu.Utilities.isAnImage(upload.original_filename)) {
      return '<img src="' + upload.url + '" width="' + upload.width + '" height="' + upload.height + '">';
    } else if (!Nilavu.SiteSettings.prevent_anons_from_downloading_files && (/\.(mov|mp4|webm|ogv|mp3|ogg|wav)$/i).test(upload.original_filename)) {
      // is Audio/Video
      if (Nilavu.CDN) {
        return Nilavu.CDN.startsWith('//') ? "http:" + Nilavu.getURLWithCDN(upload.url) : Nilavu.getURLWithCDN(upload.url);
      } else {
        return "http://" + Nilavu.BaseUrl + upload.url;
      }
    } else {
  ...
  allowsImages: function() {
    return Nilavu.Utilities.authorizesAllExtensions() ||
           (/(png|jpe?g|gif|bmp|tiff?|svg|webp|ico)/i).test(Nilavu.Utilities.authorizedExtensions());
  },

  allowsAttachments: function() {
    return Nilavu.Utilities.authorizesAllExtensions() ||
           !(/((png|jpe?g|gif|bmp|tiff?|svg|web|ico)(,\s)?)+$/i).test(Nilavu.Utilities.authorizedExtensions());
  },
