...
function relativeAgeTiny(date){
  ...
  case(distanceInMinutes >= 90 && distanceInMinutes <= 1409):
    ...
    break;
  case(Nilavu.SiteSettings.relative_date_duration === 0 && distanceInMinutes <= 525599):
    formatted = shortDateNoYear(date);
    break;
  case(distanceInMinutes >= 1410 && distanceInMinutes <= 2519):
    formatted = t("x_days", {count: 1});
    break;
  case(distanceInMinutes >= 2520 && distanceInMinutes <= ((Nilavu.SiteSettings.relative_date_duration||14) * 1440)):
