...
/**
  Creates a URL using Nilavu.getURL. It takes a fmt string just like
  fmt does.
...
export function url() {
  ...
  const computed = Em.computed(function() {
    const self = this;
    return Nilavu.getURL(format.fmt.apply(format, args.map(function (a) {
      return self.get(a);
    })));
  });
  return computed.property.apply(computed, args);
...
export function setting(name) {
  return Em.computed(function() {
    return Nilavu.SiteSettings[name];
  }).property();
}
