...
export default {
  trackClick(e) {
    ...
    if (Nilavu.Utilities.selectedText() !== "") { return false; }
    ...
    var ownLink = userId && (userId === Nilavu.User.currentProp('id')),
        trackingUrl = Nilavu.getURL("/clicks/track?url=" + encodeURIComponent(href));
    ...
    if (e.which === 3) {
      var destination = Nilavu.SiteSettings.track_external_right_clicks ? trackingUrl : href;
    ...
    if (Nilavu.SiteSettings.prevent_anons_from_downloading_files && $link.hasClass("attachment") && !Nilavu.User.current()) {
      bootbox.alert(I18n.t("post.errors.attachment_download_requires_login"));
      return false;
    }

    // If we're on the same site, use the router and track via AJAX
    if (NilavuURL.isInternal(href) && !$link.hasClass('attachment')) {
      Nilavu.ajax("/clicks/track", {
        data: {
          url: href,
          post_id: postId,
      ...
      NilavuURL.routeTo(href);
      return false;
    }

    // Otherwise, use a custom URL with a redirect
    if (Nilavu.User.currentProp('external_links_in_new_tab')) {
      var win = window.open(trackingUrl, '_blank');
      win.focus();
    } else {
      NilavuURL.redirectTo(trackingUrl);
    }
