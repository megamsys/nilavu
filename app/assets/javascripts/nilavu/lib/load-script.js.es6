...
export default function loadScript(url, opts) {
  ...
  return new Ember.RSVP.Promise(function(resolve) {
    url = Nilavu.getURL((assetPath && assetPath(url)) || url);
    ...
    if (Nilavu.CDN && url[0] === "/" && url[1] !== "/") {
      cdnUrl = Nilavu.CDN.replace(/\/$/,"") + url;
    }
    ...
    if (opts.scriptTag) {
      ...
      Nilavu.ajax({url: cdnUrl, dataType: "script", cache: true}).then(cb);
    }
  });
}
