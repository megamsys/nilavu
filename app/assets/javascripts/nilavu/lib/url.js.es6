...
const NilavuURL = Ember.Object.createWithMixins({
  ...
  replaceState: function(path) {
    if (window.history &&
        ...
        Em.run.next(function() {
          const location = NilavuURL.get('router.location');
  ...
  **/
  routeTo(path, opts) {
    ...
    if (Nilavu.get('requiresRefresh')) {
      document.location.href = Nilavu.getURL(path);
      return;
    }
    ...
    // handle prefixes
    if (path.match(/^\//)) {
      let rootURL = (Nilavu.BaseUri === undefined ? "/" : Nilavu.BaseUri);
  ...
  redirectTo: function(url) {
    window.location = Nilavu.getURL(url);
  },
    ...
    if (newTopicId) {
      ...
      if (oldTopicId === newTopicId) {
        ...
        postStream.refresh(opts).then(() => {
          ...
          NilavuURL.jumpToPost(closest, {skipIfOnScreen: true});
        });
  ...
  **/
  navigatedToHome: function(oldPath, path) {
    const homepage = Nilavu.Utilities.defaultHomepage();
  ...
  origin: function() {
    return window.location.origin + (Nilavu.BaseUri === "/" ? '' : Nilavu.BaseUri);
  },
  ...
  **/
  router: function() {
    return Nilavu.__container__.lookup('router:main');
  }.property().volatile(),
