...
import { addDecorator } from 'nilavu/widgets/post-cooked';
import ComposerEditor from 'nilavu/components/composer-editor';
import { addButton } from 'nilavu/widgets/post-menu';
import { includeAttributes } from 'nilavu/lib/transform-post';
import { addWidgetCleanCallback } from 'nilavu/components/mount-widget';
import { createWidget, decorateWidget, changeSetting } from 'nilavu/widgets/widget';
import { onPageChange } from 'nilavu/lib/page-tracker';
import { preventCloak } from 'nilavu/widgets/post-stream';
...
class PluginApi {
      ...
      if (result) {
        ...
        if (result.icon) {
          ...
          iconBody = result.emoji.split('|').map(emoji => {
            const src = Nilavu.Emoji.urlFor(emoji);
  ...
  **/
   ...
   * See `createWidget` in `nilavu/widgets/widget` for more info.
   **/
...
function getPluginApi(version) {
  if (version === "0.1" || version === "0.2") {
    if (!_pluginv01) {
      _pluginv01 = new PluginApi(version, Nilavu.__container__);
    }
    return _pluginv01;
  } else {
