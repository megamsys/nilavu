...
export default function(options) {
    ...
    if (term) {
      if (isInput) {
        ...
        if (term) {
          ...
          me.val(text);
          Nilavu.Utilities.setCaretPosition(me[0], completeStart + 1 + term.length);
  ...
  var renderAutocomplete = function() {
    ...
    if (Nilavu.Site.currentProp('mobileView') && !isInput) {
      div.css('width', 'auto');
  ...
  $(this).on('keyup.autocomplete', function() {
    ...
    var caretPosition = Nilavu.Utilities.caretPosition(me[0]);
  ...
  $(this).on('keydown.autocomplete', function(e) {
    ...
    if ((completeStart === null) && e.which === keys.backSpace && options.key) {
      c = Nilavu.Utilities.caretPosition(me[0]);
      c -= 1;
      initial = c;
      prevIsGood = true;
    ...
    if (completeStart !== null) {
      caretPosition = Nilavu.Utilities.caretPosition(me[0]);
