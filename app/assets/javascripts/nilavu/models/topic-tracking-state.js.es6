import NotificationLevels from 'nilavu/lib/notification-levels';
...
const TopicTrackingState = Nilavu.Model.extend({
  messageCount: 0,
  ...
  establishChannels() {
    ...
    const process = data => {
      ...
      if (data.message_type === "new_topic" || data.message_type === "latest") {
        const muted_category_ids = Nilavu.User.currentProp("muted_category_ids");
      ...
      if (data.payload && data.payload.category_id) {
        var category = Nilavu.Category.findById(data.payload.category_id);
  ...
  notify(data) {
    ...
    if (filterCategory && filterCategory.get("id") !== categoryId) {
      const category = categoryId && Nilavu.Category.findById(categoryId);
    ...
    if (filter === Nilavu.Utilities.defaultHomepage()) {
      const suppressed_from_homepage_category_ids = Nilavu.Site.currentProp("suppressed_from_homepage_category_ids");
  ...
  trackIncoming(filter) {
    ...
    if (split.length >= 4) {
      ...
      var category = Nilavu.Category.findSingleBySlug(split.splice(1,split.length - 3).join('/'));
  ...
  loadStates(data) {
    const states = this.states;
    const idMap = Nilavu.Category.idMap();
