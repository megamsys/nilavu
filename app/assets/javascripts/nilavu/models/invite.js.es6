const Invite = Nilavu.Model.extend({
...
Invite.reopenClass({
  ...
  create() {
    ...
    if (result.user) {
      result.user = Nilavu.User.create(result.user);
    }
    return result;
  },
  ...
  findInvitedBy(user, filter, search, offset) {
    ...
    return Nilavu.ajax("/users/" + user.get('username_lower') + "/invited.json", {data}).then(function (result) {
  ...
  findInvitedCount(user) {
    if (!user) { return Em.RSVP.resolve(); }
    return Nilavu.ajax("/users/" + user.get('username_lower') + "/invited_count.json").then(result => Em.Object.create(result.counts));
  }
