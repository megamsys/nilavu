...
const Category = RestModel.extend({
  ...
  url: function() {
    return Nilavu.getURL("/c/") + Category.slugFor(this);
  }.property('name'),
  ...
  moreTopics: function() {
    return this.get('topic_count') > Nilavu.SiteSettings.category_featured_topics;
  ...
  destroy: function() {
    return Nilavu.ajax("/categories/" + (this.get('id') || this.get('slug')), { type: 'DELETE' });
  },
  ...
  featuredTopics: function() {
    var topics = this.get('topics');
    if (topics && topics.length) {
      return topics.slice(0, Nilavu.SiteSettings.category_featured_topics || 2);
    }
  }.property('topics'),
  ...
  isUncategorizedCategory: function() {
    return this.get('id') === Nilavu.Site.currentProp("uncategorized_category_id");
  }.property('id')
});
...
Category.reopenClass({
  ...
  findUncategorized() {
    _uncategorized = _uncategorized || Category.list().findBy('id', Nilavu.Site.currentProp('uncategorized_category_id'));
  ...
  list() {
    return Nilavu.SiteSettings.fixed_category_positions ?
             Nilavu.Site.currentProp('categories') :
             Nilavu.Site.currentProp('sortedCategories');
  },

  listByActivity() {
    return Nilavu.Site.currentProp('sortedCategories');
  },

  idMap() {
    return Nilavu.Site.currentProp('categoriesById');
  },
  ...
  reloadById(id) {
    return Nilavu.ajax(`/c/${id}/show.json`);
  },

  reloadBySlug(slug, parentSlug) {
    return parentSlug ? Nilavu.ajax(`/c/${parentSlug}/${slug}/find_by_slug.json`) : Nilavu.ajax(`/c/${slug}/find_by_slug.json`);
  },
