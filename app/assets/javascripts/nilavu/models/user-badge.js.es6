...
const UserBadge = Nilavu.Model.extend({
  postUrl: function() {
  ...
  **/
  revoke: function() {
    return Nilavu.ajax("/user_badges/" + this.get('id'), {
      type: "DELETE"
    });
  }
});
...
UserBadge.reopenClass({
  ...
  createFromJson: function(json) {
    ...
    json.users.forEach(function(userJson) {
      users[userJson.id] = Nilavu.User.create(userJson);
    });
    ...
    json.topics.forEach(function(topicJson) {
      topics[topicJson.id] = Nilavu.Topic.create(topicJson);
    });
  ...
  **/
  findByUsername: function(username, options) {
    ...
    return Nilavu.ajax(url).then(function(json) {
  ...
  **/
  findByBadgeId: function(badgeId, options) {
    ...
    return Nilavu.ajax("/user_badges.json", {
      data: options
    }).then(function(json) {
      return UserBadge.createFromJson(json);
    });
  ...
  **/
  grant: function(badgeId, username, reason) {
    return Nilavu.ajax("/user_badges", {
      type: "POST",
      data: {
        username: username,
        badge_id: badgeId,
