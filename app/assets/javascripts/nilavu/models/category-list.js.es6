...
CategoryList.reopenClass({
  categoriesFrom(store, result) {
    ...
    const users = Nilavu.Model.extractByKey(result.featured_users, Nilavu.User);
    const list = Nilavu.Category.list();
    ...
    result.category_list.categories.forEach(c => {
      ...
      if (c.topics) {
        c.topics = c.topics.map(t => Nilavu.Topic.create(t));
      }
  ...
  listForParent(store, category) {
    return Nilavu.ajax(`/categories.json?parent_category_id=${category.get("id")}`).then(result => {
  ...
  list(store) {
    const getCategories = () => Nilavu.ajax("/categories.json");
