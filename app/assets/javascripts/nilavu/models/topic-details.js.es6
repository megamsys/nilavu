...
const TopicDetails = RestModel.extend({
  ...
  notificationReasonText: function() {
    ...
    return I18n.t(localeString, { username: Nilavu.User.currentProp('username_lower') });
  ...
  updateNotifications(v) {
    ...
    return Nilavu.ajax("/t/" + (this.get('topic.id')) + "/notifications", {
      type: 'POST',
  ...
  removeAllowedUser(user) {
    ...
    return Nilavu.ajax("/t/" + this.get('topic.id') + "/remove-allowed-user", {
      type: 'PUT',
