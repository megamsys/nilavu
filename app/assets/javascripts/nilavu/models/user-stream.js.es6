...
export default RestModel.extend({
  ...
  filterParam: function() {
    ...
    if (filter === Nilavu.UserAction.TYPES.replies) {
  ...
  findItems() {
    ...
    return Nilavu.ajax(findUrl, {cache: 'false'}).then( function(result) {
      if (result && result.user_actions) {
        const copy = Em.A();
        result.user_actions.forEach(function(action) {
          action.title = Nilavu.Emoji.unescape(Handlebars.Utils.escapeExpression(action.title));
