...
export default RestModel.extend({
  ...
  act(post, opts) {
    ...
    const self = this;
    return Nilavu.ajax("/post_actions", {
      type: 'POST',
      data: {
      ...
      const remaining = parseInt(data.xhr.getResponseHeader('Nilavu-Actions-Remaining') || 0);
      const max = parseInt(data.xhr.getResponseHeader('Nilavu-Actions-Max') || 0);
  ...
  undo(post) {
    ...
    return Nilavu.ajax("/post_actions/" + post.get('id'), {
      type: 'DELETE',
  ...
  deferFlags(post) {
    return Nilavu.ajax("/post_actions/defer_flags", {
      type: "POST",
