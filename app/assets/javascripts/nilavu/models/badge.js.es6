...
const Badge = RestModel.extend({
  ...
  url: function() {
    return Nilavu.getURL(`/badges/${this.get('id')}/${this.get('slug')}`);
  }.property(),
  ...
  **/
  destroy: function() {
    ...
    return Nilavu.ajax("/admin/badges/" + this.get('id'), {
      type: "DELETE"
    });
  }
});
...
Badge.reopenClass({
  ...
  **/
  findAll: function(opts) {
    ...
    return Nilavu.ajax('/badges.json' + listable).then(function(badgesJson) {
  ...
  **/
  findById: function(id) {
    return Nilavu.ajax("/badges/" + id).then(function(badgeJson) {
