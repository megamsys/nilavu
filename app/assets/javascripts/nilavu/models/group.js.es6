...
const Group = Nilavu.Model.extend({
  limit: 50,
  offset: 0,
  user_count: 0,
  owners: [],
    ...
    return Group.loadMembers(this.get("name"), offset, this.get("limit")).then(function (result) {
      ...
      self.setProperties({
        ...
        members: result.members.map(member => {
          ...
          return Nilavu.User.create(member);
        }),
        owners: result.owners.map(owner => Nilavu.User.create(owner)),
      });
    });
  },
  ...
  removeOwner(member) {
    var self = this;
    return Nilavu.ajax('/admin/groups/' + this.get('id') + '/owners.json', {
      type: "DELETE",
  ...
  removeMember(member) {
    var self = this;
    return Nilavu.ajax('/groups/' + this.get('id') + '/members.json', {
      type: "DELETE",
      data: { user_id: member.get("id") }
    }).then(function() {
      // reload member list
  ...
  create() {
    var self = this;
    return Nilavu.ajax("/admin/groups", { type: "POST", data: this.asJSON() }).then(function(resp) {
      self.set('id', resp.basic_group.id);
    });
  },

  save() {
    return Nilavu.ajax("/admin/groups/" + this.get('id'), { type: "PUT", data: this.asJSON() });
  },

  destroy() {
    if (!this.get('id')) { return; }
    return Nilavu.ajax("/admin/groups/" + this.get('id'), { type: "DELETE" });
  },
  ...
  findPosts(opts) {
    ...
    return Nilavu.ajax(`/groups/${this.get('name')}/${type}.json`, { data: data }).then(posts => {
      return posts.map(p => {
        p.user = Nilavu.User.create(p.user);
        p.topic = Nilavu.Topic.create(p.topic);
  ...
  setNotification(notification_level) {
    ...
    return Nilavu.ajax(`/groups/${this.get("name")}/notifications`, {
...
Group.reopenClass({
  findAll(opts) {
    return Nilavu.ajax("/admin/groups.json", { data: opts }).then(function (groups){
      return groups.map(g => Group.create(g));
    });
  },

  findGroupCounts(name) {
    return Nilavu.ajax("/groups/" + name + "/counts.json").then(result => Em.Object.create(result.counts));
  },

  find(name) {
    return Nilavu.ajax("/groups/" + name + ".json").then(result => Group.create(result.basic_group));
  },

  loadMembers(name, offset, limit) {
    return Nilavu.ajax('/groups/' + name + '/members.json', {
      data: {
        limit: limit || 50,
        offset: offset || 0
      }
