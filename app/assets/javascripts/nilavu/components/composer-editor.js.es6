import userSearch from 'nilavu/lib/user-search';
import { default as computed, on } from 'ember-addons/ember-computed-decorators';
import { linkSeenMentions, fetchUnseenMentions } from 'nilavu/lib/link-mentions';
import { linkSeenCategoryHashtags, fetchUnseenCategoryHashtags } from 'nilavu/lib/link-category-hashtags';
...
export default Ember.Component.extend({
    ...
    return {
      lookupAvatarByPostNumber: (postNumber, topicId) => {
        ...
        if (posts && topicId === topic.get('id')) {
          ...
          if (quotedPost) {
            return Nilavu.Utilities.tinyAvatar(quotedPost.get('avatar_template'));
          }
        }
      }
    };
  ...
  validation(reply, replyLength, missingReplyCharacters, minimumPostLength, lastValidatedAt) {
    ...
    if (replyLength < 1) {
      ...
      const tl = Nilavu.User.currentProp("trust_level");
      if (tl === 0 || tl === 1) {
    ...
    if (reason) {
      return Nilavu.InputValidation.create({ failed: true, reason, lastShownAt: lastValidatedAt });
    }
  },
  ...
  /*  this.messageBus.subscribe("/uploads/composer", upload => {
      ...
      if (upload && upload.url) {
        if (!this._xhr || !this._xhr._userCancelled) {
          const markdown = Nilavu.Utilities.getUploadMarkdown(upload);
  ...
  actions: {
    ...
    previewUpdated($preview) {
      ...
      // Paint oneboxes
      $('a.onebox', $preview).each((i, e) => Nilavu.Onebox.load(e, refresh));
