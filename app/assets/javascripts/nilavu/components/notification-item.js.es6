...
export default Ember.Component.extend({
  ...
  url: function() {
    ...
    if (topicId) {
      return Nilavu.Utilities.postUrl(it.get("slug"), topicId, it.get("post_number"));
    }

    if (it.get('notification_type') === INVITED_TYPE) {
      return Nilavu.getURL('/users/' + it.get('data.display_username'));
    }

    if (it.get('data.group_id')) {
      return Nilavu.getURL('/users/' + it.get('data.username') + '/messages/group/' + it.get('data.group_name'));
    }
  ...
  description: function() {
    ...
    if (badgeName) { return Nilavu.Utilities.escapeExpression(badgeName); }
  ...
  _markRead: function(){
    this.$('a').click(() => {
      ...
      Nilavu.setTransientHeader("Nilavu-Clear-Notifications", this.get('notification.id'));
  ...
  render(buffer) {
    ...
    text = Nilavu.Emoji.unescape(text);
