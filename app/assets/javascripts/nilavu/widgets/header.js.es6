...
createWidget('user-dropdown', jQuery.extend({
  ...
  html(attrs) {
    ...
    return h('a.c_glob.header_user_profile', { attributes: { href: currentUser.get('path'), 'data-auto-route': true } },
...
createWidget('header-link', jQuery.extend({
  ...
  html(attrs) {
    ...
    const body = [iconNode(attrs.icon, {hclasz: attrs.icon, class: attrs.align})];
    ...
    return h('a', { attributes: { href: attrs.href,
...
export default createWidget('header', {
  ...
  defaultState() {
    return { marketplacesVisible: false,
  ...
  html(attrs, state) {
    const panels = [this.attach('header-icons', { marketplacesVisible: state.marketplacesVisible,
