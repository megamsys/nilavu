...
export function avatarImg(wanted, attrs) {
  const size = Nilavu.Utilities.translateSize(wanted);
  const url = Nilavu.Utilities.avatarUrl(attrs.template, size);
  ...
  const properties = {
    attributes: { alt: '', width: size, height: size, src: Nilavu.getURLWithCDN(url), title },
    className: 'avatar'
  };
...
createWidget('post-article', {
  ...
  toggleReplyAbove() {
    ...
    if (this.attrs.mobileView) {
      ...
      if (topicUrl) {
        NilavuURL.routeTo(`${topicUrl}/${replyPostNumber}`);
      }
