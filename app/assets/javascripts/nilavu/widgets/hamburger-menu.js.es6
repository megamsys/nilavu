...
export default createWidget('hamburger-menu', {
  ...
  /*  links.push({ route: 'discovery.torpedo', className: 'torpedo-topics-link', label: 'filters.torpedo.title' });
  ...
  listCategories() {
    ...
    const isStaff = Nilavu.User.currentProp('staff');

    const categories = Nilavu.Category.list().reject((c) => {
