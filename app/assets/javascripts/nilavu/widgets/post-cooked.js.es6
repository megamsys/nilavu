import { isValidLink } from 'nilavu/lib/click-track';
import { number } from 'nilavu/lib/formatter';
...
export default class PostCooked {
  ...
  _fixImageSizes($html) {
    const maxImageWidth = Nilavu.SiteSettings.max_image_width;
    const maxImageHeight = Nilavu.SiteSettings.max_image_height;
  ...
  _toggleQuote($aside) {
    ...
    if ($aside.data('expanded')) {
      ...
      Nilavu.ajax(`/posts/by_number/${topicId}/${postId}`).then(result => {
    ...
    if (postNumber) {
      ...
      if (asideTopicId) {
        ...
        if (this.attrs.topicId === asideTopicId) {
          ...
          const asideLink = Nilavu.getURL("/t/via-quote/") + asideTopicId + "/" + postNumber;
