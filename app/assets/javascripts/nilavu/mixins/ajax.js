/**
  ...
  respect Nilavu paths and the run loop.
**/
var _trackView = false;
var _transientHeader = null;

Nilavu.Ajax = Em.Mixin.create({
  ...
  **/
  ajax: function() {
    ...
    if (args.success || args.error) {
      throw "Nilavu.ajax should use promises";
    }
    ...
    var performAjax = function(resolve, reject) {
      ...
      if (_trackView && (!args.type || args.type === "GET")) {
        ...
        args.headers['Nilavu-Track-View'] = "true";
      }
     ...
     */
      ...
      args.error = function(xhr, textStatus, errorThrown) {
        ...
        if (xhr.status === 403 && xhr.responseText === "['BAD CSRF']") {
          Nilavu.Session.current().set('csrfToken', null);
        }
      ...
      if (args.dataType === "script") {
        args.headers['Nilavu-Script'] = true;
      }
      ...
      ajax = $.ajax(Nilavu.getURL(url), args);
    };
    ...
    if(args.type && args.type.toUpperCase() !== 'GET' && !Nilavu.Session.currentProp('csrfToken')){
      promise = new Ember.RSVP.Promise(function(resolve, reject){
        ajax = $.ajax(Nilavu.getURL('/session/csrf'), {cache: false})
           .success(function(result){
              Nilavu.Session.currentProp('csrfToken', result.csrf);
