...
export function categoryBadgeHTML(category, opts) {
  ...
  if ((!category) ||
        (!opts.allowUncategorized &&
         Em.get(category, 'id') === Nilavu.Site.currentProp("uncategorized_category_id") &&
         Nilavu.SiteSettings.suppress_uncategorized_badge
        )
     ) return "";

  var description = get(category, 'description_text'),
      restricted = get(category, 'read_restricted'),
      url = opts.url ? opts.url : Nilavu.getURL("/c/") + Nilavu.Category.slugFor(category),
  ...
  if (!opts.hideParent) {
    parentCat = Nilavu.Category.findById(get(category, 'parent_category_id'));
  }
  ...
  extraClasses = Nilavu.SiteSettings.category_style ? Nilavu.SiteSettings.category_style + extraClasses : extraClasses;
