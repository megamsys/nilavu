import { filterQueryParams, findTopicList } from 'nilavu/routes/build-topic-route';
import { queryParams } from 'nilavu/controllers/discovery-sortable';
import TopicList from 'nilavu/models/topic-list';
import PermissionType from 'nilavu/models/permission-type';
import CategoryList from 'nilavu/models/category-list';
  ...
  return Nilavu.Route.extend({
    ...
    _setupNavigation(category) {
      const noSubcategories = params && !!params.no_subcategories,
            filterMode = `c/${Nilavu.Category.slugFor(category)}${noSubcategories ? "/none" : ""}/l/${filter}`;
    ...
    _createSubcategoryList(category) {
      ...
      if (Em.isNone(category.get('parentCategory')) && Nilavu.SiteSettings.show_subcategory_list) {
    ...
    _retrieveTopicList(category, transition) {
      const listFilter = `c/${Nilavu.Category.slugFor(category)}/l/${filter}`,
