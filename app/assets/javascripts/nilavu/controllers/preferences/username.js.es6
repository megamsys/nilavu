import { setting, propertyEqual } from 'nilavu/lib/computed';
...
export default Ember.Controller.extend({
  ...
  checkTaken: function() {
    if( this.get('newUsername') && this.get('newUsername').length < this.get('minLength') ) {
      ...
      Nilavu.User.checkUsername(this.get('newUsername'), undefined, this.get('content.id')).then(function(result) {
        if (result.errors) {
      ...
      return bootbox.confirm(I18n.t("user.change_username.confirm"), I18n.t("no_value"), I18n.t("yes_value"), function(result) {
        if (result) {
          ...
          self.get('content').changeUsername(self.get('newUsername')).then(function() {
            NilavuURL.redirectTo("/users/" + self.get('newUsername').toLowerCase() + "/preferences");
          }, function() {
            // error
            self.set('error', true);
