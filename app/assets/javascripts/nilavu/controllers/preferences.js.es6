  ...
  actions: {
    ...
    save() {
      ...
      return model.save().then(() => {
        if (Nilavu.User.currentProp('id') === model.get('id')) {
          Nilavu.User.currentProp('name', model.get('name'));
        }
        model.set('bio_cooked', Nilavu.Markdown.cook(Nilavu.Markdown.sanitize(model.get('bio_raw'))));
        this.set('saved', true);
