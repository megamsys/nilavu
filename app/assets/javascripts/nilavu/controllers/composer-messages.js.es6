...
export default Ember.ArrayController.extend({
  ...
  actions: {
    ...
    popup(message) {
      ...
      let templateName = message.get('templateName');
  ...
  groupsMentioned(groups) {
    ...
    groups.forEach(group => {
      const msg = I18n.t('composer.group_mentioned', {
        ...
        group_link: Nilavu.getURL(`/group/${group.name}/members`)
      });
      this.send("popup",
        Em.Object.create({
