...
export function uploadTranslate(key, options) {
  options = options || {};
  if (Nilavu.Utilities.allowsAttachments()) { key += "_with_attachments"; }
...
export default Ember.Controller.extend(ModalFunctionality, {
  ...
  @computed
  uploadIcon() {
    return Nilavu.Utilities.allowsAttachments() ? "upload" : "picture-o";
  },

  @computed('controller.local')
  tip(local) {
    const source = local ? "local" : "remote";
    const authorized_extensions = Nilavu.Utilities.authorizesAllExtensions() ? "" : `(${Nilavu.Utilities.authorizedExtensions()})`;
