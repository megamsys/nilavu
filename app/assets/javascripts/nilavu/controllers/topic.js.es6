...
export default Ember.Controller.extend(BufferedContent, {
      ...
      if (user.get('staff') && replyCount > 0) {
        bootbox.dialog(I18n.t("post.controls.delete_replies.confirm", {count: replyCount}), [
          ...
          {label: I18n.t("post.controls.delete_replies.yes_value"),
           'class': 'btn-primary',
            callback() {
              Nilavu.Post.deleteMany([post], [post]);
  ...
  canChangeOwner: function() {
    if (!Nilavu.User.current() || !Nilavu.User.current().admin) return false;
  ...
  showStarButton: function() {
    return Nilavu.User.current() && !this.get('model.isPrivateMessage');
  ...
  deleteTopic() {
    this.get('content').destroy(Nilavu.User.current());
  },
