...
import { MAX_MESSAGE_LENGTH } from 'nilavu/models/post-action-type';
...
export default Ember.Controller.extend(ModalFunctionality, {
  ...
  submitEnabled: function() {
    ...
    if (selected.get('is_custom_flag')) {
      ...
      return len >= Nilavu.SiteSettings.min_private_message_post_length &&
  ...
  canTakeAction: function() {
    ...
    return Nilavu.User.currentProp('staff');
  ...
  canDeleteSpammer: function() {
    ...
    if (Nilavu.User.currentProp('staff') && this.get('selected.name_key') === 'spam') {
  ...
  fetchUserDetails() {
    if (Nilavu.User.currentProp('staff') && this.get('model.username')) {
