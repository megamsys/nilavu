...
export default Ember.Controller.extend({
  ...
  customMessageLengthClasses: function() {
    return (this.get('message.length') < Nilavu.SiteSettings.min_private_message_post_length) ? "too-short" : "ok";
  }.property('message.length'),

  customMessageLength: function() {
    var len = this.get('message.length') || 0;
    var minLen = Nilavu.SiteSettings.min_private_message_post_length;
    if (len === 0) {
