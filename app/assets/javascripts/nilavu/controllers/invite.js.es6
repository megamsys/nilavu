...
export default Ember.Controller.extend(ModalFunctionality, {
  ...
  isAdmin: function(){
    return Nilavu.User.currentProp("admin");
  }.property(),
  ...
  disabled: function() {
    ...
    if (!this.get('invitingToTopic') && !Nilavu.Utilities.emailValid(emailOrUsername)) return true;
    // normal users (not admin) can't invite users to private topic via email
    if (!this.get('isAdmin') && this.get('isPrivateTopic') && Nilavu.Utilities.emailValid(emailOrUsername)) return true;
    // when inviting to private topic via email, group name must be specified
    if (this.get('isPrivateTopic') && Ember.isEmpty(this.get('model.groupNames')) && Nilavu.Utilities.emailValid(emailOrUsername)) return true;
  ...
  disabledCopyLink: function() {
    ...
    const emailOrUsername = this.get('emailOrUsername').trim();
    // email must be valid
    if (!Nilavu.Utilities.emailValid(emailOrUsername)) return true;
    // normal users (not admin) can't invite users to private topic via email
    if (!this.get('isAdmin') && this.get('isPrivateTopic') && Nilavu.Utilities.emailValid(emailOrUsername)) return true;
    // when inviting to private topic via email, group name must be specified
    if (this.get('isPrivateTopic') && Ember.isEmpty(this.get('model.groupNames')) && Nilavu.Utilities.emailValid(emailOrUsername)) return true;
  ...
  showCopyInviteButton: function() {
    return (!Nilavu.SiteSettings.enable_sso && !this.get('isMessage'));
  }.property('isMessage'),
  ...
  showGroups: function() {
    return this.get('isAdmin') && (Nilavu.Utilities.emailValid(this.get('emailOrUsername')) || this.get('isPrivateTopic') || !this.get('invitingToTopic')) && !Nilavu.SiteSettings.enable_sso && Nilavu.SiteSettings.enable_local_logins && !this.get('isMessage');
  }.property('isAdmin', 'emailOrUsername', 'isPrivateTopic', 'isMessage', 'invitingToTopic'),

  // Instructional text for the modal.
  inviteInstructions: function() {
    if (Nilavu.SiteSettings.enable_sso || !Nilavu.SiteSettings.enable_local_logins) {
      ...
      if (this.get('isPrivateTopic') && !this.get('isAdmin')) {
        ...
        } else if (Nilavu.Utilities.emailValid(this.get('emailOrUsername'))) {
  ...
  groupFinder(term) {
    const Group = require('nilavu/models/group').default;
  ...
  placeholderKey: function() {
    return (Nilavu.SiteSettings.enable_sso || !Nilavu.SiteSettings.enable_local_logins) ?
  ...
  actions: {
    ...
    createInvite() {
      const Invite = require('nilavu/models/invite').default;
      const self = this;
