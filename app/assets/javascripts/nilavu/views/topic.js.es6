...
const TopicView = Ember.View.extend(AddCategoryClass, AddArchetypeClass, Scrolling, {
  ...
  browseMoreMessage: function() {
    ...
    var opts = { latestLink: "<a href=\"" + Nilavu.getURL("/latest") + "\">" + I18n.t("topic.view_latest_topics") + "</a>" },
        category = this.get('topic.category');

    if(category && Em.get(category, 'id') === Nilavu.Site.currentProp("uncategorized_category_id")) {
      category = null;
    }
