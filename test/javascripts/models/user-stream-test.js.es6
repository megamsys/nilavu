...
module("Nilavu.UserStream");

test('basics', function(){
  var user = Nilavu.User.create({id: 1, username: 'eviltrout'});
...
test('filterParam', function() {
  ...
  stream.set('filter', Nilavu.UserAction.TYPES.likes_given);
  equal(stream.get('filterParam'), Nilavu.UserAction.TYPES.likes_given);

  stream.set('filter', Nilavu.UserAction.TYPES.replies);
