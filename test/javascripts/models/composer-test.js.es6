...
test('missingReplyCharacters', function() {
  Nilavu.SiteSettings.min_first_post_length = 40;
  ...
  missingReplyCharacters('hi', false, false, Nilavu.SiteSettings.min_post_length - 2, 'too short public post');
  missingReplyCharacters('hi', false, true,  Nilavu.SiteSettings.min_first_post_length - 2, 'too short first post');
  missingReplyCharacters('hi', true, false,  Nilavu.SiteSettings.min_private_message_post_length - 2, 'too short private message');
});
...
test('missingTitleCharacters', function() {
  ...
  missingTitleCharacters('hi', false, Nilavu.SiteSettings.min_topic_title_length - 2, 'too short post title');
  missingTitleCharacters('z', true,  Nilavu.SiteSettings.min_private_message_title_length - 1, 'too short pm title');
});
...
test("Title length for regular topics", function() {
  Nilavu.SiteSettings.min_topic_title_length = 5;
  Nilavu.SiteSettings.max_topic_title_length = 10;
  const composer = createComposer();
...
test("Title length for private messages", function() {
  Nilavu.SiteSettings.min_private_message_title_length = 5;
  Nilavu.SiteSettings.max_topic_title_length = 10;
...
test('editingFirstPost', function() {
  ...
  const post = Nilavu.Post.create({id: 123, post_number: 2});
...
test('initial category when uncategorized is allowed', function() {
  Nilavu.SiteSettings.allow_uncategorized_topics = true;
...
test('initial category when uncategorized is not allowed', function() {
  Nilavu.SiteSettings.allow_uncategorized_topics = false;
  const composer = openComposer({action: 'createTopic', draftKey: 'asfd', draftSequence: 1});
...
test("Title length for static page topics as admin", function() {
  ...
  const post = Nilavu.Post.create({id: 123, post_number: 2, static_doc: true});
  composer.setProperties({post: post, action: Composer.EDIT });
