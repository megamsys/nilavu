...
var buildPost = function(args) {
  return Nilavu.Post.create(_.merge({
    id: 1,
    can_delete: true,
    version: 1
  }, args || {}));
...
test('defaults', function() {
  var post = Nilavu.Post.create({id: 1});
...
test('new_user', function() {
  var post = Nilavu.Post.create({trust_level: 0});
...
test('firstPost', function() {
  var post = Nilavu.Post.create({post_number: 1});
...
test('updateFromPost', function() {
  var post = Nilavu.Post.create({
    post_number: 1,
    raw: 'hello world'
  });

  post.updateFromPost(Nilavu.Post.create({
    raw: 'different raw',
...
test('destroy by staff', function() {
  var user = Nilavu.User.create({username: 'staff', staff: true}),
...
test('destroy by non-staff', function() {
  var originalCooked = "this is the original cooked value",
      user = Nilavu.User.create({username: 'evil trout'}),
