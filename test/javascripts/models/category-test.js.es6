...
test('slugFor', function(){
  ...
  const slugFor = function(cat, val, text) {
    equal(Nilavu.Category.slugFor(cat), val, text);
  };
...
test('findBySlug', function() {
  ...
  sandbox.stub(Nilavu.Category, 'list').returns(categoryList);

  deepEqual(Nilavu.Category.findBySlug('darth'), darth, 'we can find a category');
  deepEqual(Nilavu.Category.findBySlug('luke', 'darth'), luke, 'we can find the other category with parent category');
  deepEqual(Nilavu.Category.findBySlug('熱帶風暴畫眉'), hurricane, 'we can find a category with CJK slug');
  deepEqual(Nilavu.Category.findBySlug('뉴스피드', '熱帶風暴畫眉'), newsFeed, 'we can find a category with CJK slug whose parent slug is also CJK');
  deepEqual(Nilavu.Category.findBySlug('时间', 'darth'), time, 'we can find a category with CJK slug whose parent slug is english');
  deepEqual(Nilavu.Category.findBySlug('bah', '熱帶風暴畫眉'), bah, 'we can find a category with english slug whose parent slug is CJK');
...
test('findSingleBySlug', function() {
  ...
  deepEqual(Nilavu.Category.findSingleBySlug('darth'), darth, 'we can find a category');
  deepEqual(Nilavu.Category.findSingleBySlug('darth/luke'), luke, 'we can find the other category with parent category');
  deepEqual(Nilavu.Category.findSingleBySlug('熱帶風暴畫眉'), hurricane, 'we can find a category with CJK slug');
  deepEqual(Nilavu.Category.findSingleBySlug('熱帶風暴畫眉/뉴스피드'), newsFeed, 'we can find a category with CJK slug whose parent slug is also CJK');
  deepEqual(Nilavu.Category.findSingleBySlug('darth/时间'), time, 'we can find a category with CJK slug whose parent slug is english');
  deepEqual(Nilavu.Category.findSingleBySlug('熱帶風暴畫眉/bah'), bah, 'we can find a category with english slug whose parent slug is CJK');
});
...
test('findByIds', function() {
  ...
  sandbox.stub(Nilavu.Category, 'idMap').returns(categories);
  deepEqual(Nilavu.Category.findByIds([1,2,3]), _.values(categories));
});
