...
import { relativeAge, autoUpdatingRelativeAge, updateRelativeAge, breakUp, number } from 'nilavu/lib/formatter';
...
test("formating tiny dates", function() {
  ...
  var originalValue = Nilavu.SiteSettings.relative_date_duration;
  Nilavu.SiteSettings.relative_date_duration = 7;
  equal(formatDays(7), "7d");
  equal(formatDays(8), shortDate(8));

  Nilavu.SiteSettings.relative_date_duration = 1;
  equal(formatDays(1), "1d");
  equal(formatDays(2), shortDate(2));

  Nilavu.SiteSettings.relative_date_duration = 0;
  equal(formatMins(0), "< 1m");
  equal(formatMins(2), "2m");
  equal(formatMins(60), "1h");
  ...
  Nilavu.SiteSettings.relative_date_duration = null;
  equal(formatDays(1), '1d');
  equal(formatDays(14), '14d');
  equal(formatDays(15), shortDate(15));

  Nilavu.SiteSettings.relative_date_duration = 14;
  ...
  Nilavu.SiteSettings.relative_date_duration = originalValue;
});
