...
test("does not update badge clicks on my own link", function() {
  sandbox.stub(Nilavu.User, 'currentProp').withArgs('id').returns(314);
  badgeClickCount('with-badge', 1);
});

test("does not update badge clicks in my own post", function() {
  sandbox.stub(Nilavu.User, 'currentProp').withArgs('id').returns(3141);
...
test("right clicks are tracked", function() {
  Nilavu.SiteSettings.track_external_right_clicks = true;
  trackRightClick();
...
test("tracks custom urls when opening in another window", function() {
  var clickEvent = generateClickEventOn('a');
  sandbox.stub(Nilavu.User, "currentProp").withArgs('external_links_in_new_tab').returns(true);
  ok(!track(clickEvent));
