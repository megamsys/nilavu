...
const loggedIn = () => !!Nilavu.User.current();
...
export default function() {
  ...
  const server = new Pretender(function() {
    ...
    this.get("/groups/nilavu/topics.json", () => {
      return response(200, fixturesByUrl['/groups/nilavu/posts.json']);
    });

    this.get("/groups/nilavu/mentions.json", () => {
      return response(200, fixturesByUrl['/groups/nilavu/posts.json']);
    });

    this.get("/groups/nilavu/messages.json", () => {
      return response(200, fixturesByUrl['/groups/nilavu/posts.json']);
    });
