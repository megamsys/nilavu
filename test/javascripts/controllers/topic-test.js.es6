...
test("toggledSelectedPost", function() {
  var tc = this.subject({ model: buildTopic() }),
      post = Nilavu.Post.create({id: 123, post_number: 2}),
      postStream = tc.get('model.postStream');

  postStream.appendPost(post);
  postStream.appendPost(Nilavu.Post.create({id: 124, post_number: 3}));
...
test("Select Replies when present", function() {
  var topic = buildTopic(),
      tc = this.subject({ model: topic }),
      p1 = Nilavu.Post.create({id: 1, post_number: 1, reply_count: 1}),
      p2 = Nilavu.Post.create({id: 2, post_number: 2}),
      p3 = Nilavu.Post.create({id: 2, post_number: 3, reply_to_post_number: 1});
