...
Nilavu.rootElement = '#ember-testing';
Nilavu.setupForTesting();
Nilavu.injectTestHelpers();
Nilavu.start();
...
var origDebounce = Ember.run.debounce,
    ...
    flushMap = require('nilavu/models/store', null, null, false).flushMap,
    ScrollingDOMMethods = require('nilavu/mixins/scrolling', null, null, false).ScrollingDOMMethods,
    _NilavuURL = require('nilavu/lib/url', null, null, false).default,
    server;
...
function resetSite() {
  ...
  Nilavu.Site.resetCurrent(Nilavu.Site.create(siteAttrs));
}
...
QUnit.testStart(function(ctx) {
  ...
  Nilavu.SiteSettings = dup(Nilavu.SiteSettingsOriginal);
  Nilavu.BaseUri = "";
  Nilavu.BaseUrl = "localhost";
  Nilavu.Session.resetCurrent();
  Nilavu.User.resetCurrent();
  resetSite();

  _NilavuURL.redirectedTo = null;
  _NilavuURL.redirectTo = function(url) {
    _NilavuURL.redirectedTo = url;
  };
...
window.asyncTestNilavu = helpers.asyncTestNilavu;
